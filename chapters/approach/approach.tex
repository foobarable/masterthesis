...
\subsection{Repetition}
 ...
 \begin{tabular}{|c|l|l|}
  ...
  \hline
  k & A constant that defines the maximal score & \ref{def:scorewithmaximalscore} \\
  L & TODO: definition missing (different label?) & \ref{def:scoreadjusted} \\
  T & Threshold score for subalignments & \ref{def:treshold} \\
  g & Gap penalty with $g>0$ & \ref{def:gappenalty} \\
  f & Number of occurrences a task must have in all user sessions to be replaced & \ref{def:minoccurencecount} \\
  \hline
 \end{tabular}
